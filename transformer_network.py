...
class GNT(nn.Module):
    def __init__(self,in_feat_ch=32, posenc_dim=3, viewenc_dim=3, ret_alpha=False):
        super(GNT, self).__init__()
        self.netwidth = 64
        self.trans_depth = 4
        self.rgbfeat_fc = nn.Sequential(
            nn.Linear(in_feat_ch + 3, self.netwidth),
            nn.ReLU(),
        ...
        for i in range(self.trans_depth):
            ...
            view_trans = Transformer2D(
                dim=self.netwidth,
                ff_hid_dim=int(self.netwidth * 4),
                ff_dp_rate=0.1,
                attn_dp_rate=0.1,
            )
            ...
            if i % 2 == 0:
                q_fc = nn.Sequential(
                    nn.Linear(self.netwidth + posenc_dim + viewenc_dim, self.netwidth),
                    nn.ReLU(),
                    nn.Linear(self.netwidth, self.netwidth),
                )
            else:
        ...
        self.norm = nn.LayerNorm(self.netwidth)
        self.rgb_fc = nn.Linear(self.netwidth, 3)
        self.relu = nn.ReLU()
